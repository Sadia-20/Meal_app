// it makes an array if it does not exist in local storage
// this array will be used to store favorite list
if (localStorage.getItem("favouritesList") == null) {
    localStorage.setItem("favouritesList", JSON.stringify([]));
}

// it fetches meals from the API and returns it
    ...
    meals.then(data=>{
        if (data.meals) {
            data.meals.forEach((element) => {
                ...
                //if meal is in the favorite list this will be the code for that
                if (isFav) {
                    html += `
                ...
                `;
                //else ,if it's not in favorite list then below will be the code for that 
                } else {
                    html += `
